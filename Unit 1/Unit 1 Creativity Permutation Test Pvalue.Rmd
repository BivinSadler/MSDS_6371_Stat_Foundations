...
This code calculates and records the difference in means: xbarIntrinsic - xbarExtrinsic = 4.14
...
In the study, we drew one sample of 47 and the difference in the sample means between those that recieved the Intrinsic motivatoion and those that received the Extrinsic motivation found to be 4.14 points. The pvalue will then the probability that, if the Intrnsic motivation does not have an effect on the scores with respect to the Extrinsic motivation, that we would observe a result as extreme ore more extreme than 4.14 points.  

The good news is, we have an estimate of how this statistic behaves (is distributed) in just this case ... in the case where the Intrinsic Motivation has no effect on the creativity scores with respect to the Extrinsic Motiviation.  We just built that distribution (aka the "Null distribution.")

Therefore we simpy have to count how many of the 10,000 differences of means we calculated earlier are greater than than 4.14 or less than -4.14.  
...
In this case the pvalue is .0864 and thus 8.64% of the differences in sample means from the 10,000 simulated experiements as extreme or more extreme than 4.14 points thus this is not deemed to be an "unlikely" result to have observed by chance if the null hypothesis were true.  Therefore, we will not reject the null hypothesis (Fail To Reject Ho) that the mean score of the population of those that take the "New" method is different from the mean score of the population that take the "Traditional" method.  
...
**The Conclusion: There is strong evidence to suggest that the mean creativity score of the Intrinsic Group is greater than that of the Extrinsic group.  (pvalue < .001).**
